...
export function MissionControls({ mission }: Readonly<{ mission: { _id: Id<"missions">; title: string; summary: string; lifecycle: "active" | "archived" | "pendingDeletion" | "deletedTombstone"; currentVersion: number; role: string } }>) {
  ...
  async function run(intent: string, action: (idempotencyKey: string) => Promise<{ currentVersion: number }>) { if (pending) return; if (baseVersion !== mission.currentVersion) { setFeedback("This Mission changed elsewhere. Close Manage Mission to load the latest details before saving."); return; } setPending(true); setFeedback(null); keys.current[intent] ??= crypto.randomUUID(); try { const result = await action(keys.current[intent]); setBaseVersion(result.currentVersion); setFeedback("Mission updated."); } catch { setFeedback("The Mission changed elsewhere. The live Mission has been refreshed; review and try again."); keys.current[intent] = crypto.randomUUID(); } finally { setPending(false); } }
