...
- The provider-independent room-session kernel and development-only Ably adapter have deterministic fake-clock and fake-transport/client evidence only. Strict schemas, authenticated identity binding, exact capability-family routing, local rate governance, bounded receiver caches, and privacy-safe disabled telemetry now fail closed, but no provider connectivity, forced disconnect, browser/background behavior, live token refresh, enabled telemetry sink, multi-browser network recovery, or load/fan-out behavior is claimed.
