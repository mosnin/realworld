...
- The provider-independent room-session kernel, development-only Ably adapter, signal governor, privacy telemetry, and browser lifecycle policy have deterministic fake-clock, fake-client, and injected-source evidence only. Strict schemas, authenticated identity binding, exact capability-family routing, local rate governance, bounded receiver caches, privacy-safe disabled telemetry, hidden/focus publication gating, and ordered offline teardown now fail closed, but no DOM lifecycle adapter, provider connectivity, forced provider disconnect, real background-tab behavior, live token refresh, enabled telemetry sink, multi-browser network recovery, or load/fan-out behavior is claimed.
...
- Twenty browser journeys pass in GitHub Actions run `30338939551` against an ephemeral Convex deployment with real private-alpha account creation, including stable contention-free Pulse ordering, deadline-bearing Call response history and resolution persistence, durable Pulse history, the durable Proof verification loop, durable Fracture recovery, reactive two-context Call participation and responses, the durable owner Call and Move workflows, Constitution edit/re-edit/archive/restore persistence, real multi-Mission creation/selection/reload/archive, mobile room entry, durable Mission lifecycle, archived read-only controls, deny-by-default scoped discovery, live two-context movement, a concurrent owner/builder OCC race with visible recovery, and a true browser disconnect/reconnect cycle. The browser-lifecycle slice has no changed browser surface, uses no real DOM lifecycle source, and keeps its Ably path disabled/offline.
