...
- The provider-independent room-session kernel and development-only Ably adapter have deterministic fake-clock and fake-transport/client evidence only. Strict per-kind schemas and exact capability-family routing now fail closed, but no provider connectivity, forced disconnect, browser/background behavior, live token refresh, telemetry, rate limiting, multi-browser network recovery, or load/fan-out behavior is claimed.
