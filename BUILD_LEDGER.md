...
- CI and production build: passed in GitHub Actions run `30354653672`
- Signed-in responsive shell: passed with local ephemeral Convex Auth
- Authorization and transient-state boundaries: 151 automated tests pass locally and in hosted CI across Mission, guarded realtime token issuance, provider-free authenticated token-response normalization and lifecycle dependency composition, injected scope/grant-bound development Ably transport construction with no fallback runtime SDK import or constructor, bounded client-factory resolution and late-client disposal, stable provider-client contract preflight, hostile getter and time-of-check/time-of-use containment, generic provider-operation and connection-failure normalization, bounded provider-operation timeouts, eager and late compensation, ordered idempotent cleanup, uncertain-presence retry, startup-error precedence, strict signal schemas, authenticated realtime identity, publish budgets, privacy-safe telemetry, bounded receiver caches, browser visibility/focus policy, strict DOM lifecycle feature gating, durable room-readiness authorization, provider-free session-factory binding, hostile dependency containment, scope and grant-version disposal, ordered offline/online lifecycle composition, the development-only Ably adapter, offline room-session lifecycle, contention-free event ordering, Constitution, Move, Call administration, participation and response history, Fracture recovery, Proof verification, Pulse history, membership, invitation, room scopes, lifecycle, expiry, dependencies, capacity, OCC, and idempotency behavior
