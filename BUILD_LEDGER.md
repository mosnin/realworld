...
- Calls now have a durable owner-operated canvas journey and a tested Convex participant kernel. Participant controls on the canvas, two-context Call collaboration, deadlines, resolution summaries, Ably presence, and real network/load behavior are not implemented or claimed.
...
- Sixteen browser journeys pass locally and in GitHub Actions run `30326912024` against an ephemeral Convex deployment with real private-alpha account creation, including the durable owner Call and Move workflows, Constitution edit/re-edit/archive/restore persistence, real multi-Mission creation/selection/reload/archive, mobile room entry, durable Mission lifecycle, archived read-only controls, deny-by-default scoped discovery, live two-context movement, a concurrent owner/builder OCC race with visible recovery, and a true browser disconnect/reconnect cycle. These journeys do not yet exercise Call participation.
...
- CI and production build: passed in GitHub Actions run `30326912024`
- Signed-in responsive shell: passed with local ephemeral Convex Auth
- Authorization boundaries: fifty-one automated tests pass locally and in hosted CI across Mission, Constitution, Move, Call administration and participation, membership, invitation, room scopes, lifecycle, expiry, dependencies, capacity, OCC, and idempotency behavior
