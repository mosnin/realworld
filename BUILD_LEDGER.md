...
- Calls are currently a durable backend kernel only. The canvas still renders a static Call fixture, and participant capacity, joining, responses, deadlines, resolution summaries, and two-context Call interaction are not implemented or claimed.
...
- CI and production build: passed in GitHub Actions run `30324958122`
- Signed-in responsive shell: passed with local ephemeral Convex Auth
- Authorization boundaries: forty-four automated tests pass locally and in hosted CI across Mission, Constitution, Move, Call, membership, invitation, room scopes, lifecycle, expiry, dependencies, OCC, and idempotency behavior
