...
- The current Mission World and Workshop still use fixture content for occupants, activity, artifacts, Proof, and Pulse. Room identity, titles, lifecycle, coordinates, Moves, Calls, and Fractures are now durable Convex state; presentation preferences and layout lock intentionally remain personal and browser-local.
...
- Fractures now have a durable owner/reporter-operated lifecycle and room-anchored canvas controls against ephemeral Convex. Proof and Pulse remain fixtures; cloud-preview collaboration, Ably presence, and real network/load behavior are not implemented or claimed.
...
- CI and production build: passed in GitHub Actions run `30328926086`
- Signed-in responsive shell: passed with local ephemeral Convex Auth
- Authorization boundaries: fifty-five automated tests pass locally and in hosted CI across Mission, Constitution, Move, Call administration and participation, Fracture recovery, membership, invitation, room scopes, lifecycle, expiry, dependencies, capacity, OCC, and idempotency behavior
