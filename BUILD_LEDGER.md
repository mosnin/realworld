...
- The current Mission World and Workshop still use fixture content for occupants, activity, artifacts, and Pulse. Room identity, titles, lifecycle, coordinates, Moves, Calls, Fractures, and Proofs are now durable Convex state; presentation preferences and layout lock intentionally remain personal and browser-local.
...
- Calls now have a durable owner-operated lifecycle, participant controls on the canvas, and a real two-context reactive collaboration journey against ephemeral Convex. Deadlines, response history, resolution summaries, Ably presence, cloud-preview collaboration, and real network/load behavior are not implemented or claimed.
- Fractures now have a durable owner/reporter-operated lifecycle and room-anchored canvas controls against ephemeral Convex. Pulse remains a fixture; cloud-preview collaboration, Ably presence, and real network/load behavior are not implemented or claimed.
- Proofs now have a durable submit/edit/reject/resubmit/verify lifecycle and room-anchored canvas controls against ephemeral Convex. Pulse remains a fixture; independent reviewer collaboration, cloud-preview collaboration, Ably presence, and real network/load behavior are not implemented or claimed.
...
- Nineteen browser journeys pass locally and in GitHub Actions run `30329956746` against an ephemeral Convex deployment with real private-alpha account creation, including the durable Proof verification loop, durable Fracture recovery, reactive two-context Call participation and responses, the durable owner Call and Move workflows, Constitution edit/re-edit/archive/restore persistence, real multi-Mission creation/selection/reload/archive, mobile room entry, durable Mission lifecycle, archived read-only controls, deny-by-default scoped discovery, live two-context movement, a concurrent owner/builder OCC race with visible recovery, and a true browser disconnect/reconnect cycle.
...
- CI and production build: passed in GitHub Actions run `30329956746`
- Signed-in responsive shell: passed with local ephemeral Convex Auth
- Authorization boundaries: fifty-eight automated tests pass locally and in hosted CI across Mission, Constitution, Move, Call administration and participation, Fracture recovery, Proof verification, membership, invitation, room scopes, lifecycle, expiry, dependencies, capacity, OCC, and idempotency behavior
