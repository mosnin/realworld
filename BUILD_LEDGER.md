...
- Twenty-two browser journeys pass and one background-visibility case is explicitly skipped in GitHub Actions run `30340669786` against an ephemeral Convex deployment. The authenticated Mission evidence remains intact, and the provider-free lifecycle diagnostic additionally proves default-disabled zero construction, explicit test opt-in, and a real Chromium offline stop/online restart cycle. The diagnostic route is 404 outside the test environment, uses an injected fake session, and keeps the Ably/provider path unconstructed; headless background-tab visibility/focus behavior is not claimed.
