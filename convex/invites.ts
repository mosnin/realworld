...
export const acceptInvite = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now(); if (invite.state !== "active" || invite.expiresAt <= now || invite.uses >= invite.maxUses) throw new Error("Invite is unavailable");
    const scope = `invite:${invite._id}:principal:${tokenIdentifier}`; const fingerprint = JSON.stringify({ command: "acceptInvite", hash }); const prior = await ctx.db.query("operationReceipts").withIndex("by_scope_and_idempotency_key", q => q.eq("scope", scope).eq("idempotencyKey", args.idempotencyKey)).unique();
