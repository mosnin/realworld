...
export const createInvite = mutation({
  ...
  handler: async (ctx, args) => {
    const membership = await requireActiveMembership(ctx, args.missionId); await requireWritableMission(ctx, args.missionId); requireRole(membership, ["owner", "steward"]);
    const now = Date.now(); if (args.expiresAt <= now || args.expiresAt > now + 30 * 86400000 || args.maxUses < 1 || args.maxUses > 100 || args.roomIds.length < 1 || args.roomIds.length > 8) throw new Error("Invalid invite constraints");
...
export const acceptInvite = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await requireWritableMission(ctx, invite.missionId); const now = Date.now(); const scope = `invite:${invite._id}:principal:${tokenIdentifier}`; const fingerprint = JSON.stringify({ command: "acceptInvite", hash }); const prior = await ctx.db.query("operationReceipts").withIndex("by_scope_and_idempotency_key", q => q.eq("scope", scope).eq("idempotencyKey", args.idempotencyKey)).unique();
...
export const revokeInvite = mutation({
  ...
  returns: v.object({ inviteId: v.id("invites"), eventId: v.id("missionEvents") }),
  handler: async (ctx, args) => {
    const invite = await ctx.db.get(args.inviteId); if (!invite) throw new Error("Not found"); const membership = await requireActiveMembership(ctx, invite.missionId); await requireWritableMission(ctx, invite.missionId); requireRole(membership, ["owner", "steward"]);
