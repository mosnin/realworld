...
export const createMissionFromTemplate = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const missionId = await ctx.db.insert("missions", { ownerPrincipalId: principalId, slug: args.slug, title: args.title.trim(), summary: template.summary, visibility: "private", lifecycle: "active", currentVersion: 1, eventSequence: 1, templateKey: args.templateKey, createdAt: now, updatedAt: now, schemaVersion: 1 });
